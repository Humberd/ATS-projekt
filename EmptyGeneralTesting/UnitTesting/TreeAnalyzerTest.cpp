...
TEST_CLASS(TreeAnalyzerTest) {
	...
	TEST_METHOD(TreeAnalyzer_analyzeModifiesTable) {
		...
		Assert::IsTrue(procedureMap.at("Moon").size() == 2); //Moon(x,y)
		Assert::IsTrue(procedureMap.at("Moon").at(0) == "x");
		Assert::IsTrue(procedureMap.at("Moon").at(1) == "y");

		Assert::IsTrue(procedureMap.at("Sun").size() == 2); //Sun(x,z)
		Assert::IsTrue(procedureMap.at("Sun").at(0) == "x");
		Assert::IsTrue(procedureMap.at("Sun").at(1) == "z");

		Assert::IsTrue(procedureMap.at("Earth").size() == 4); //d,t,x + Sun(x,z) = Earth(d,t,x,z)
		...
		Assert::IsTrue(procedureMap.at("Earth").at(3) == "z");

		Assert::IsTrue(procedureMap.at("Planet").size() == 7); //a,d,m,x,y,z + Sun(x,z) + Earth(d,t,x,z) + Moon(x,y) = Planet(a,d,m,t,x,y,z)
