...
TEST_CLASS(QueryVariablesValidatorTest) {
	...
	TEST_METHOD(QueryVariablesValidator_addTypesToVariablesAndParams_Valid) {
		vector<DeclaredVariable*> declaredVariables = {
			new DeclaredVariable(DeclarationKeywords::ASSIGN, "a"),
			new DeclaredVariable(DeclarationKeywords::STATEMENT, "s"),
			new DeclaredVariable(DeclarationKeywords::PROCEDURE, "Grazyna"),
			new DeclaredVariable(DeclarationKeywords::CALL, "foo"),
			new DeclaredVariable(DeclarationKeywords::WHILE, "b")
		};
		...
		QueryVariablesValidator* validator = new QueryVariablesValidator;
		validator->addTypesToVariablesAndParams(declaredVariables, queryRequest);

		Assert::IsTrue(queryRequest->getReturnRequest()->getVariables().at(0)->getType() == DeclarationKeywords::ASSIGN);
		Assert::IsTrue(queryRequest->getReturnRequest()->getVariables().at(1)->getType() == DeclarationKeywords::STATEMENT);

		Assert::IsTrue(queryRequest->getMethodRequests().at(0)->getLeftParam()->getVariableType() == DeclarationKeywords::CALL);
		Assert::IsTrue(queryRequest->getMethodRequests().at(0)->getRightParam()->getVariableType() == DeclarationKeywords::PROCEDURE);

		Assert::IsTrue(queryRequest->getWithRequests().at(0)->getLeftSideVariable()->getType() == DeclarationKeywords::ASSIGN);
		Assert::IsTrue(queryRequest->getWithRequests().at(0)->getRightSideVariable()->getType() == DeclarationKeywords::STATEMENT);
